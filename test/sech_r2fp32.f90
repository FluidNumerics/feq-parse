program test
  ...
  exit_code = sech_r2fp32()
  stop exit_code

contains

integer function sech_r2fp32() result(r)
  use FEQParse
  use iso_fortran_env
  implicit none
  ...
  do j = 1,N
    do i = 1,N
      ...
      fexact(i,j) = 2.0_real32/(exp(x(i,j,1)) + exp(-x(i,j,1)))* &
                    2.0_real32/(exp(x(i,j,2)) + exp(-x(i,j,2)))
    end do
  end do
