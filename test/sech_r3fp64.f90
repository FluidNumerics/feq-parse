program test
  ...
  exit_code = sech_r3fp64()
  stop exit_code

contains

integer function sech_r3fp64() result(r)
  use FEQParse
  use iso_fortran_env
  implicit none
  ...
  do k = 1,N
    do j = 1,N
      do i = 1,N
        fexact(i,j,k) = 2.0_real64/(exp(x(i,j,k,1)) + exp(-x(i,j,k,1)))* &
                        2.0_real64/(exp(x(i,j,k,2)) + exp(-x(i,j,k,2)))* &
                        2.0_real64/(exp(x(i,j,k,3)) + exp(-x(i,j,k,3)))
      end do
    end do
  end do
