program test
  ...
  exit_code = monadic_r3fp32()
  stop exit_code

contains

integer function monadic_r3fp32() result(r)
  ...
  do k = 1,N
    do j = 1,N
      do i = 1,N
        fexact(i,j,k) = -(x(i,j,k,1) + 1.0_real32)*(x(i,j,k,2) + 1.0_real32)*(x(i,j,k,3) + 1.0_real32)
      end do
    end do
  end do
