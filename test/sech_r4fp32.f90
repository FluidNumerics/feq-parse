program test
  ...
  exit_code = sech_r4fp32()
  stop exit_code

contains

  integer function sech_r4fp32() result(r)
    use FEQParse
    use iso_fortran_env
    implicit none
    ...
    do l = 1,M
      do k = 1,N
        do j = 1,N
          do i = 1,N
            fexact(i,j,k,l) = 2.0_real32/(exp(x(i,j,k,l,1))+exp(-x(i,j,k,l,1)))* &
                              2.0_real32/(exp(x(i,j,k,l,2))+exp(-x(i,j,k,l,2)))* &
                              2.0_real32/(exp(x(i,j,k,l,3))+exp(-x(i,j,k,l,3)))
          enddo
        enddo
      enddo
    enddo
