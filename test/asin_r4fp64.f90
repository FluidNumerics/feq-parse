program test
    ...
    exit_code = asin_r4fp64()
    stop exit_code

contains

    integer function asin_r4fp64() result(r)
        use FEQParse
        use iso_fortran_env
        implicit none
        ...
        eqChar = 'f = asin( x )*asin( y )*asin( z )*asin( t )'
        ...
        do l = 1, M
            do k = 1, N
                do j = 1, N
                    do i = 1, N
                        fexact(i, j, k, l) = asin(x(i, j, k, l, 1)) * asin(x(i, j, k, l, 2)) * asin(x(i, j, k, l, 3)) * asin(x(i, j, k, l, 4))
                    end do
                end do
            end do
        end do
