program test
    ...
    exit_code = tan_r4fp32()
    stop exit_code

contains

    integer function tan_r4fp32() result(r)
        use FEQParse
        use iso_fortran_env
        implicit none
        ...
        do l = 1, M
            do k = 1, N
                do j = 1, N
                    do i = 1, N
                        fexact(i, j, k, l) = tan(0.5_real32 * pi * x(i, j, k, l, 1)) * tan(0.5_real32 * pi * x(i, j, k, l, 2)) * tan(0.5_real32 * pi * x(i, j, k, l, 3))
                    end do
                end do
            end do
        end do
